...
def login():
  ...
  if request.method == "POST":  #theres a few things i need to change
    ...
    user = user.lower()
    with open(ROOT_DIR + "logins.json", 'r') as file:
      db = json.load(file)
    if user in db:
...
def register():
  if request.method == "POST":
    ...
    with open(ROOT_DIR + "logins.json", 'w') as file:
      json.dump(logins, file)
    with open(ROOT_DIR + "misc.json", 'r') as file:
      misc = json.load(file)
    ...
    with open(ROOT_DIR + "misc.json", 'w') as file:
      json.dump(misc, file)
...
def chat():
  ...
  if request.method == "POST":
    ...
    with open(ROOT_DIR + "chat.json", 'w') as file:
...
def chatuserrefresh():
  ...
  with open(ROOT_DIR + "users.json", 'r') as f:
...
def bot_read():
  token = request.args.get('token')
  with open(ROOT_DIR + 'chat.json', 'r') as f:
    messages = json.load(f)
    f.close()
  with open(ROOT_DIR + 'misc.json', 'r') as f:
    misc = json.load(f)
    f.close()
  if token in misc:
